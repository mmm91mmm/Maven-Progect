...
<script>
	...
	<form:form action="/admin/size" method="get" modelAttribute="filter">
		<c:forEach items="${param}" var="parameter">
			<c:forEach items="${parameter.value}" var="value">
	...
	<table>
		...
		<c:forEach items="${page.content}" var="size">
			<tr>
				<td>${size.name}</td>
				<td>
					<a href="/admin/size/delete/${size.id}?page=${page.number+1}&size=${page.size}&sort=${param['sort']}&search=${param['search']}">delete</a>
				</td>
				<td>
					<a href="/admin/size/update/${size.id}?page=${page.number+1}&size=${page.size}&sort=${param['sort']}&search=${param['search']}">update</a>
				</td>
			</tr>
		</c:forEach>
		<tr>
	...
	<div class="col-md-6">
			<div class="dropdown">
				...
				<ul class="dropdown-menu">
					<custom:sort innerHtml="Size name asc"
						paramValue="name" />
					<custom:sort innerHtml="Size name desc"
