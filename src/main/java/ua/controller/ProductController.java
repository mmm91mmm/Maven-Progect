...
public class ProductController {
	...
	public String showProductForUser(Model model, @PageableDefault(10) Pageable pageable,
	...
	@RequestMapping("/user/productAll")
	public String showProductForAll(Model model, @PageableDefault(10) Pageable pageable,
			@ModelAttribute(value="filter") ProductFilterForm form){
		model.addAttribute("page", productService.findAll(pageable, form));
		model.addAttribute("countrys", countryService.findAll());
		model.addAttribute("manufacturers", manufacturerService.findAll());
		...
		model.addAttribute("sizes", sizeService.findAll());
		model.addAttribute("productSize", productService.findAll());
		return "userProductAll";
	}
	
		
	@RequestMapping("/admin/product")
	public String showProduct(Model model, @PageableDefault(10) Pageable pageable,
			@ModelAttribute(value="filter") ProductFilterForm form){
		model.addAttribute("page", productService.findAll(pageable, form));
		model.addAttribute("countrys", countryService.findAll());
		model.addAttribute("manufacturers", manufacturerService.findAll());
	...
	@RequestMapping("/admin/product/OneProduct/{id}")
	public String showOneProduct(Model model, @PageableDefault(10) Pageable pageable,
			@ModelAttribute(value="filter") ProductFilterForm form, @PathVariable int id){
		model.addAttribute("page", productService.findById(id));
		model.addAttribute("countrys", countryService.findAll());
		model.addAttribute("manufacturers", manufacturerService.findAll());
		model.addAttribute("subcategorys", subcategoryService.findAll());
		model.addAttribute("colors", colourService.findAll());
		...
		return "adminOneProduct";
	}
	
	
	
	@RequestMapping(value= "/admin/product/saveOrder", method=RequestMethod.POST)
	public String addBuy(Principal principal, @RequestParam int productId, @RequestParam String sizeName){
		MyOrder myOrder = new MyOrder();
		myOrderService.save(productId,myOrder,sizeName);	
...
//Principal
	...
	public String deleteProduct(@PathVariable int id, @PageableDefault(10) Pageable pageable,
			@ModelAttribute(value="filter") ProductFilterForm form){
